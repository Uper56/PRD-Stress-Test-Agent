...
const GROUPS: { key: keyof Verdict; severity: string; label: string }[] = [
  { key: 'p0_blockers', severity: 'P0', label: 'P0 阻断项' },
  { key: 'p1_concerns', severity: 'P1', label: 'P1 关注项' },
  { key: 'p2_suggestions', severity: 'P2', label: 'P2 建议' },
];
  ...
  return (
    <div className={`px-card px-card--accent ${styles.wrap}`}>
      <div className={styles.headline}>
        <span className={`px-label ${styles.kicker}`}>Supervisor 裁决</span>
        ...
        {GROUPS.map(({ key, severity, label }) => {
          ...
          return (
            <div key={key} className={`${styles.column} ${styles[`col${severity.toLowerCase()}`]}`}>
              ...
              {items.length === 0 ? (
                <div className={styles.none}>—— 暂无 ——</div>
              ) : (
      ...
      {(verdict.conflict_resolutions?.length ?? 0) > 0 && (
        <div className={styles.conflicts}>
          <div className={`px-label ${styles.conflictsHead}`}>分歧裁决</div>
          <ul className={styles.items}>
