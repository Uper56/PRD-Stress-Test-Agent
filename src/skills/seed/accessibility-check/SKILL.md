...
description: Use this skill WHENEVER the PRD specifies a new UI surface — screen, modal, dialog, form, table, chart, tooltip, button, icon — without explicitly addressing accessibility. The PRD MUST cover WCAG AA contrast, keyboard navigability, screen-reader labels, non-color-only affordances, motion preferences, and touch-target sizing. Apply firmly — accessibility retrofits cost 10–100× more than designing them in.
version: "1.0"
created_by: seed
injected_into:
  - design
trigger_keywords:
  ...
  - tooltip
  - icon
trigger_semantic: PRD specifies a new UI surface (screen, modal, form, button) without calling out accessibility requirements.
confidence: 0.78
---
